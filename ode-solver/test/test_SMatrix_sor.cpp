...
void run_test(const SMatrix& A, const std::vector<double>& b, double omega = 1.0) {
    std::cout << "----------------------------------------\n";
    std::cout << "SOR 红黑并行测试" << "\n";
    ...
    auto [x, it] = SMatrix::sor_rb(A, b, omega);
    ...
    std::cout << "松弛因子 = " << omega << "，迭代数：" << it + 1 << std::endl;
    std::cout << "----------------------------------------\n\n";
}
...
int main() {
    ...
    run_test(A, b, 1.5);
    run_test(A, b, 1.9);
    run_test(A, b, 2/(1+std::sin(std::acos(-1.0)/(N+1))));
