...
void run_test_gmres(const std::string& precond_name, const SMatrix& A, const std::vector<double>& b, int restart) 
{
    std::cout << "----------------------------------------\n";
...
void run_test_bicgstab(const std::string& precond_name, const SMatrix& A, const std::vector<double>& b) 
{
    std::cout << "----------------------------------------\n";
    std::cout << "Testing Preconditioner: " << precond_name << "\n";
    ...
    // 调用你的 bicgstab 函数 (专门用于非对称矩阵)
    auto [x, it] = SMatrix::bicgstab(A, b, precond_name);
...
int main() {
    ...
    std::vector<double> restart = {80, 100, 140};
    ...
    std::cout << "======================== 一、gmres + amg ==========================\n";

    for (uint32_t i = 0; i < restart.size(); i++)
    {
        run_test_gmres("amg", A, b, restart[i]);  
    }

    // std::cout << "======================== 二、gmres + ilu ==========================\n";

    // for (uint32_t i = 0; i < restart.size(); i++)
    // {
    //     run_test_gmres("ilu", A, b, restart[i]);  
    // }

    // std::cout << "====================== 三、bicgstab + amg =========================\n";

    // run_test_bicgstab("amg", A, b);

    // std::cout << "====================== 四、bicgstab + ilu =========================\n";

    // run_test_bicgstab("ilu", A, b);
